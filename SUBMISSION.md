...
**Oracles:** on devnet, `push_price_pyth` sets a major's mark from Pyth (`PriceUpdateV2`, Full verification, fixed feed per mint, freshness and confidence bounds) and requires Switchboard On-Demand's SOL/USD (Coinbase + Kraken jobs, pinned by queue and feed hash) to agree within 100 bps; after that, the manual crank cannot move the feed. `check:pyth` on devnet: 17/17 (program slot 497,557,294). The app's mark crank uses it, and masked-eight.vercel.app has served this build since 06:44 UTC. Three two-wallet devnet duels (06:07–06:51 UTC, the last match `Ez3eWqLs…`) settled with Pyth-owned feeds. In `main` since 20:49 UTC (36381b6).
