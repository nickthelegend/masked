...
| Z8 | Public tapes API | `/api/tapes` lists the devnet program's tapes; `/api/tapes/<match>` replays each side onto the chain's own pnl | **P** — https://market-proxy-production.up.railway.app/api/tapes 200, 9 tapes; `3Mr9KAFq…` replays to 793 / −19 bps = pnl_a / pnl_b; `not-a-key` 400; a non-match 404; 04:40 UTC (acbde4f) |
| Z9 | Share card, share page, share intent | The card is drawn from the tape; the page names it in og:image; the app's button carries the page | **P** — `/og/tape/3Mr9KAFq….png` 200 image/png 1200×630, inspected; `/t/3Mr9KAFq…` og:image = the https card URL, twitter:card summary_large_image, redirect to the app's /tape; masked-eight.vercel.app bundle `entry-a51b4abf…` carries SHARE ON X and the Railway base; on :8082 the button opened x.com/intent/post with the /t/ URL; 04:46–04:50 UTC (029625f). How X renders the card is untested (needs a post from an account) |
| Z10 | Pyth marks for majors, checked by Switchboard, on devnet | `push_price_pyth` sets a major's mark from Pyth only when Switchboard's SOL/USD agrees, locks the crank out, prices its own priority fee; bad inputs refused | **P** — `check:pyth` 17/17 against slot 497,557,294, 05:22–05:26 UTC: Switchboard SOL/USD $101.815 vs Pyth $101.804 (1 bps); USDC mark = recomputation from the same Pyth bytes; SOL leg 1e15; fee 5,060 = 5,000 + 60 with a floor, 5,000 and no compute-budget instruction at the bare estimate; OracleOwnsFeed, OracleUpdateNotNewer, OracleWrongFeed, OracleAccountInvalid, NotAMajor, SecondOracleWrongFeed, SecondOracleStale (125 s); both rounds settled (branch pyth-majors a65deb8). OraclesDisagree: unit tests only |
| Z11 | TEE suite after the program upgrades | The ER privacy lifecycle still passes on devnet-tee with each new binary | **P** — `er-privacy` FOGDUEL_ER=tee: 7 passing, 0 failing against slot 497,552,279 (05:11:32–05:12:04 UTC) and again against slot 497,557,294 (05:26:25–05:26:53 UTC) |
| Z12 | Two-wallet duel on devnet with oracle-priced majors (branch pyth-majors) | The app, not a script, prices a SOL and a USDC leg through push_price_pyth with Switchboard kept fresh by the keeper; the round trades on the TEE and settles | **P** — duel `AvPBJkpg…`, Chrome on 127.0.0.1:4196 (creator, USDC) and localhost:4196 (joiner, SOL), burner keys funded by transfer: both feeds Pyth-owned (3CsZFpDT…, 4hTzHWnf…); USDC mark = recomputation from Pyth bytes (9,826,081,780,947), re-pushed at 9,826,461,024,276 when Pyth republished; keeper 5 refreshes, Switchboard ≤59 s old; joiner long on its TEE book; settled 06:12:44, winner creator, 0.098 SOL paid, tape and share card served. PYTH badge fix (137f307) seen in a second round, match `EuxBRQfR…`: the joiner's ticker read "MARK 0.009830401◎ · pyth" and its USDC feed was Pyth-owned on chain at 9,830,400,948,813 (= Pyth recomputation); a background creator tab still showed "jupiter" until its own crank ticked; that round settled 06:19:52 (winner `ANQxLfVu`, 0.098 SOL paid) with its tape and share card served. That "jupiter" was a defect (fixed 4b75f17, badge read from the feed on chain): in a third round (`Ez3eWqLs…`) the hidden, never-clicked creator tab read "pyth" at 06:50:07, the second its feed turned Pyth-owned on chain, and the joiner flipped from "jupiter" to "pyth" when its USDC feed did (9,866,819,340,527 = Pyth recomputation) |
| Z13 | Oracles disagree, refused on chain | With Switchboard SOL/USD more than 100 bps from Pyth, `push_price_pyth` is refused with `OraclesDisagree` after every earlier check passes | **P** — isolated local validator (127.0.0.1:18999), real program binary, oracle accounts dumped from devnet with fresh timestamps and Switchboard set +3% (104.297 vs Pyth 101.259): match `6NaLZtGh…` live, push refused `OraclesDisagree`; 06:56:44–52 UTC (`scripts/check-oracles-disagree.sh`, branch pyth-majors) |
