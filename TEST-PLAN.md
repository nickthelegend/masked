...
| Z10 | Pyth marks for majors on devnet | `push_price_pyth` sets a major's mark from Pyth and locks the crank out; bad inputs refused | **P** — `check:pyth` 12/12 against the upgraded program (slot 497,552,279): USDC mark = recomputation from the same Pyth bytes, 4 bps from Jupiter; SOL leg 1e15; OracleOwnsFeed, OracleUpdateNotNewer, OracleWrongFeed, OracleAccountInvalid, NotAMajor; both rounds settled; 05:09–05:11 UTC (branch pyth-majors 8aea692) |
| Z11 | TEE suite after the program upgrade | The ER privacy lifecycle still passes on devnet-tee with the new binary | **P** — `er-privacy` FOGDUEL_ER=tee: 7 passing, 0 failing; 05:11:32–05:12:04 UTC |
